...
bool DirectfnInterface<ParticularKernel>::set_Gaussian_orders_4(const size_t N1_in, const size_t N2_in,
	...
	if (!up_kernel_->set_zw_N(N1_, up_z1_, up_w1_)) { return false;}
	if (!up_kernel_->set_zw_N(N2_, up_z2_, up_w2_)) { return false; }
	if (!up_kernel_->set_zw_N(N3_, up_z3_, up_w3_)) { return false; }
	if (!up_kernel_->set_zw_N(N4_, up_z4_, up_w4_)) { return false; }
...
//template <typename ParticularKernel >
//bool DirectfnInterface<ParticularKernel>::set_zw_N_(const size_t Nn, unique_ptr<double[]> & up_zn, unique_ptr<double[]> & up_wn)  const noexcept {
    //up_zn.reset(new double[Nn]);
    //up_wn.reset(new double[Nn]);
	//return up_quadrature_->set_zw_N_(int(Nn), up_zn, up_wn);
